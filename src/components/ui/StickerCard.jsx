...
// Total copias: missing=0, have=1, duplicate=2+extra (cap MAX_EXTRAS=2 → max 4).
...
export default function StickerCard({
  ...
  const handleClick = () => {
    if (typeof onToggle === 'function') onToggle(card.id)
  }
  return (
    <button
      type="button"
      ...
      aria-label={`#${card.num} ${card.name}, ${statusLabel(status, extra).toLowerCase()}`}
    ...
    >
      ...
      <span className={s.statusChip} aria-hidden="true">
        ...
        <span className={s.statusLabel}>{statusLabel(status, extra)}</span>
      </span>
    </button>
  )
}
